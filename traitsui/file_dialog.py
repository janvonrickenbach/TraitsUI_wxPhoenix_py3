...
from traits.api import (Bool, Button, CList, Event, File, HasPrivateTraits,
                        Instance, Int, Interface, Property, Str,
                        cached_property, provides)

from traits.trait_base import user_name_for

from .api import (CodeEditor, FileEditor, HGroup, HSplit, Handler,
                  HistoryEditor, ImageEditor, InstanceEditor, Item, UIInfo,
                  VGroup, VSplit, View, spring)
...
class FileInfo(MFileDialogModel):
    ...
    view = View(
        VGroup(
            Item(
                'size', label='File size', style='readonly'),
            Item(
                'atime', label='Last access', style='readonly'),
            Item(
                'mtime', label='Last modified', style='readonly'),
            Item(
                'ctime', label='Created at', style='readonly'),
...
class TextInfo(MFileDialogModel):
    ...
    view = View(
        Item(
            'text', style='readonly', show_label=False, editor=CodeEditor()))
...
class ImageInfo(MFileDialogModel):
    ...
    view = View(
        VGroup(
            VGroup(
                Item(
                    'width', style='readonly'),
                Item(
                    'height', style='readonly'),
                label='Image Dimensions',
                show_border=True),
            VGroup(
                Item(
                    'image', show_label=False, editor=ImageEditor()),
                label='Image',
                show_border=True,
                springy=True)))
...
class CreateDirHandler(Handler):
    ...
    view = View(
        VGroup(
            HGroup(
                Item(
                    'handler.dir_name', label='Name'),
                Item(
                    'handler.ok',
                    show_label=False,
                    enabled_when="handler.dir_name.strip() != ''"),
                Item(
                    'handler.cancel', show_label=False), ),
            HGroup(
                Item(
                    'handler.message',
                    show_label=False,
    ...
    view = View(
        VGroup(
            HGroup(
                ...
                Item(
                    'handler.message', style='readonly'),
                show_labels=False),
            HGroup(
                spring,
                Item('handler.ok'),
...
class OpenFileDialog(Handler):
    ...
    def object_create_changed(self, info):
        ...
        CreateDirHandler().edit_traits(
            context=self, parent=info.create.control)
    ...
    def open_file_view(self):
        ...
        if len(self.extensions) > 0:
            ...
            item = klass(
                item,
                VSplit(
                    id='splitter2', springy=True, *items),
                id='splitter')
                ...
                HGroup(
                    ...
                    Item(
                        ...
                        editor=HistoryEditor(
                            entries=self.entries, auto_set=True),
                        springy=True),
                    Item(
                        'ok',
                        id='ok',
    ...
    def _file_already_exists(self):
        ...
        FileExistsHandler(message=("The file '%s' already exists.\nDo "
                          ...
                          basename(self.file_name)).edit_traits(
                              context=self, parent=self.info.ok.control).set(
                                  parent=self.info.ui)
