...
class ImageInfo(HasPrivateTraits):
    ...
    def _get_image_info_code(self):
        data = dict(
            [(name, repr(value))
             for name, value in self.get('name', 'image_name', 'description',
                                         'category', 'keywords').iteritems()])
...
class ImageVolumeInfo(HasPrivateTraits):
    ...
    def _get_image_volume_info_code(self):
        data = dict(
            [(name, repr(value))
             for name, value in self.get('description', 'copyright', 'license',
                                         'image_names').iteritems()])
...
class ImageVolume(HasPrivateTraits):
    ...
    def _get_image_volume_code(self):
        data = dict(
            [(name, repr(value))
             for name, value in self.get('description', 'category', 'keywords',
                                         'aliases', 'time_stamp').iteritems()])
        data['info'] = ',\n'.join(
