...
class EditorFactory(HasPrivateTraits):
    ...
    def _get_toolkit_editor(cls, class_name):
        ...
        """
        editor_factory_classes = [
            factory_class for factory_class in cls.mro()
        ...
        for index in range(len(editor_factory_classes)):
            try:
                ...
                editor_file_name = os.path.basename(sys.modules[
                    factory_class.__module__].__file__)
                object_ref = ':'.join(
                    [editor_file_name.split('.')[0], class_name])
