...
from traits.api import (Any, Bool, Callable, Enum, Event, Float, Instance,
                        List, Str, Trait, TraitPrefixList)
...
from .ui_traits import (AButton, ATheme, AnObject, Buttons, DockStyle,
                        EditorStyle, ExportType, HelpId, Image, SequenceTypes,
                        ViewStatus)
...
AKind = Trait(
    'live',
    TraitPrefixList('panel', 'subpanel', 'modal', 'nonmodal', 'livemodal',
...
class View(ViewElement):
    ...
    def set_content(self, *values):
        ...
        for value in values:
            ...
            elif (isinstance(value, str) and (value[:1] == '<') and
                  (value[-1:] == '>')):
