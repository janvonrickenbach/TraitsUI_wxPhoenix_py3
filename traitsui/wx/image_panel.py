...
class ImagePanel(ThemedWindow):
    ...
    def _paint_fg(self, dc):
        ...
        if (text != '') and self.can_show_text:
            ...
            if (tdyp <= xtop) and (xtop >= xbottom):
                ...
                ty = (
                    wdy + xtop + label.top - xbottom - label.bottom - tdy) // 2
                if xleft >= xright:
                    cl = label.left
    ...
    def _adjusted_size_of(self, dx, dy):
        ...
        return wx.Size(
            dx + min(slice.left, slice.xleft) + min(
                slice.right, slice.xright) + content.left + content.right,
            dy + min(slice.top, slice.xtop) + min(
                slice.bottom, slice.xbottom) + content.top + content.bottom)
...
class ImageSizer(wx.PySizer):
    ...
    def CalcMin(self):
        ...
        return wx.Size(
            max(slice.left + slice.right, slice.xleft + slice.xright +
                content.left + content.right + dx),
            max(slice.top + slice.bottom, slice.xtop + slice.xbottom +
