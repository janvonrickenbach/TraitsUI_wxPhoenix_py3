...
class _ThemedSliderEditor(Editor):
    ...
    def dispose(self):
        ...
        disconnect_no_id(self.control, wx.EVT_ERASE_BACKGROUND, wx.EVT_PAINT,
                         wx.EVT_SET_FOCUS, wx.EVT_LEFT_DOWN, wx.EVT_LEFT_UP,
                         wx.EVT_MOTION, wx.EVT_MOUSEWHEEL, wx.EVT_ENTER_WINDOW,
                         wx.EVT_LEAVE_WINDOW, wx.EVT_SIZE)
    ...
    def _on_paint(self, event):
        ...
        dx = max(0,
                 min(wdx - 2,
                     int(
                         round(((wdx - 3) * (self.value - self.low)) / (
                             self.high - self.low)))))
    ...
    def _set_focus(self, event):
        ...
        """
        if ((not self._ignore_focus) and (self._x is None) and
            (self._text is None)):
            self._pop_up_text()
    ...
    def _mouse_wheel(self, event):
        ...
        if self._in_window:
            ...
            self.update_object(
                min(max(self.value + increment * delta, self.low), self.high))
