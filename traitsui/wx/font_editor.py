...
class ToolkitEditorFactory(BaseToolkitEditorFactory):
    ...
    def to_wx_font(self, editor):
        ...
        """
        font = editor.value
        return wx.Font(font.GetPointSize(),
                       font.GetFamily(),
                       font.GetStyle(),
                       font.GetWeight(),
    ...
    def str_font(self, font):
        ...
        return '%s point %s%s%s' % (font.GetPointSize(), font.GetFaceName(),
                                    style, weight)
...
class SimpleFontEditor(BaseSimpleEditor):
    ...
    def popup_editor(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            self.value = self.factory.from_wx_font(dialog.GetFontData()
                                                   .GetChosenFont())
            self.update_editor()
...
class CustomFontEditor(Editor):
    ...
    def init(self, parent):
        ...
        control = self._facename = wx.Choice(panel, -1,
                                             wx.Point(0, 0),
        ...
        if self.factory.show_style:
            self._style = wx.Choice(panel, -1,
                                    wx.Point(0, 0), wx.Size(-1, -1), Styles)
        ...
        if self.factory.show_weight:
            self._weight = wx.Choice(panel, -1,
                                     wx.Point(0, 0), wx.Size(-1, -1), Weights)
            sizer2.Add(self._weight, 1, wx.EXPAND | wx.LEFT, 3)
            self._weight(wx.EVT_CHOICE, self.update_object_parts)
    ...
    def update_object_parts(self, event):
        ...
        font = wx.Font(
            point_size, wx.DEFAULT, style, weight, faceName=facename)
