...
class TableEditor(Editor, BaseTableEditor):
    ...
    def init(self, parent):
        ...
        self.auto_add = (factory.auto_add and
        ...
        if (factory.edit_view == ' ') or (not row_mode):
            ...
            dw.SetSizer(
                DockSizer(contents=DockSection(
                    contents=contents,
            ...
            if len(factory.filters) > 0:
                view = View(
                    [
                        Item(
                            'filter<250>{View}',
                            editor=factory._filter_editor), '_', Item(
    ...
    def set_extended_selection(self, *pairs):
        ...
        grid_selections = [
            TraitGridSelection(
                obj=object, name=name) for object, name in pairs
        ]
    ...
    def _get_selected_indices(self):
        ...
        if sm == 'rows':
            ...
            return list(
                set([row_col[0] for row_col in self.selected_cell_indices]))
    ...
    def _get_selected_items(self):
        ...
        if sm == 'rows':
            ...
            return list(
                set([item_name[0] for item_name in self.selected_cells]))
    ...
    def on_delete(self):
        ...
        if n > 0:
            if self.in_row_mode:
                self.set_selection(list(set([items[i] for i in indices])))
            else:
                self.set_extended_selection(
                    list(
                        set([(items[indices[i]], values[i][1])
                             for i in range(n)])))
        else:
    ...
    def on_prefs(self):
        ...
        columns = self.columns[:]
        columns.extend([
            c for c in (self.factory.columns + self.factory.other_columns)
            if c not in columns
        ])
        self.edit_traits(
            parent=self.control,
...
class TableSearchHandler(Handler):
    ...
    def handler_select_changed(self, info):
        ...
        if info.initialized:
            ...
            filter = info.object.filter
            items = [
                item for item in editor.model.get_filtered_items()
                if filter(item)
            ]
