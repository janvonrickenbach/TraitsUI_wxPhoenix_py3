...
class Panel(BaseDialog):
    ...
    def __init__(self, ui, parent, allow_buttons):
        ...
        if ((title != '') and
            ...
            sw_sizer.Add(heading_text(
                cpanel, text=title).control,
                         0,
                         wx.EXPAND)
        ...
        if (allow_buttons and ((nbuttons != 1) or
...
def show_help(ui, button):
    ...
    for item in group.get_content(False):
        if not item.is_spacer():
            fields.append(template.item_help % (escape(item.get_label(ui)),
...
def fill_panel_for_group(panel,
                         group,
                         ui,
...
class FillPanel(object):
    ...
    def __init__(self, panel, group, ui, suppress_label, is_dock_window,
        ...
        if (is_dock_window or create_panel or is_scrolled_panel or
            (id != '') or (theme is not None) or (group.visible_when != '') or
            (group.enabled_when != '')):
        ...
        if group.show_border:
            ...
            if label != '':
                self.sizer.Add(heading_text(
                    panel, text=label).control,
                               0,
    ...
    def add_items(self, content, panel, sizer):
        ...
        for item in content:
            ...
            if name == '':
                ...
                if label != '':
                    ...
                    if theme is not None:
                        ...
                        item_sizer.Add(label, 0, wx.TOP | wx.BOTTOM |
                                       wx.EXPAND, 3)
            ...
            item_sizer.Add(control, growable, flags | layout_style |
    ...
    def create_label(self,
        ...
        control = ImageText(
            parent, item.label_theme, label + suffix, border=border)
    ...
    def _add_emphasis(self, control):
        ...
        if emphasis_font is None:
            ...
            emphasis_font = wx.Font(font.GetPointSize() + 1,
                                    font.GetFamily(), font.GetStyle(), wx.BOLD)
