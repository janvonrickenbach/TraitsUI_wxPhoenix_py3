...
class ImageSlice ( HasPrivateTraits ):
    ...
    def _analyze_bitmap ( self ):
        ...
        self.bg_color = eval('wx.Colour(r,g,b)')#int(((0x10000 * r) + (0x100 * g) + b), base=16)
    ...
    def _is_equal ( self, data, x0, y0, x1, y1, dx, dy ):
        ...
        """
        return (abs( data[ int(y0): int(y0 + dy), int(x0): int(x0 + dx) ] -
                     data[ int(y1): int(y1 + dy), int(x1): int(x1 + dx) ] ).sum() < 0.10 * dx * dy)
