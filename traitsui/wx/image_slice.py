...
class ImageSlice(HasPrivateTraits):
    ...
    def fill(self, dc, x, y, dx, dy, transparent=False):
        ...
        if n == 1:
            ...
            pdxs = [(dxs[0], dxs[0]), (dxs[1], max(0, tdx)), (0, 0), (0, 0),
                    (dxs[2], dxs[2])]
        else:
        ...
        if n == 1:
            ...
            pdys = [(dys[0], dys[0]), (dys[1], max(0, tdy)), (0, 0), (0, 0),
                    (dys[2], dys[2])]
        else:
    ...
    def _analyze_bitmap(self):
        ...
        data = reshape(fromstring(bytes(image.GetData()), uint8), (dy, dx, 3))
        ...
        self.top = int(min(dy // 2, self.dys[0]))
        self.bottom = int(min(dy // 2, self.dys[-1]))
        self.left = int(min(dx // 2, self.dxs[0]))
        self.right = int(min(dx // 2, self.dxs[-1]))
        ...
        x, y = (dx // 2), (dy // 2)
        r, g, b = data[int(y), int(x)]
        self.bg_color = eval(
            'wx.Colour(r,g,b)'
        )  #int(((0x10000 * r) + (0x100 * g) + b), base=16)
        ...
        if self.xtop >= self.xbottom:
            ...
            self.label_color = self._find_best_color(data, x, dy -
                                                     (self.xbottom / 2) - 1)
    ...
    def _find_best_borders(self, data):
        ...
        self.xleft = int(min(self.left, left))
        self.xright = int(min(self.right, dx - right - 1))
        self.xtop = int(min(self.top, top))
        self.xbottom = int(min(self.bottom, dy - bottom - 1))
    ...
    def _is_equal(self, data, x0, y0, x1, y1, dx, dy):
        ...
        """
        return (abs(data[int(y0):int(y0 + dy), int(x0):int(x0 + dx)] - data[
            int(y1):int(y1 + dy), int(x1):int(x1 + dx)]).sum() < 0.10 * dx * dy
                )
