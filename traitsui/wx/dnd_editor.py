...
class SimpleEditor(Editor):
    ...
    def init(self, parent):
        ...
        if disabled_image is not None:
            ...
            data = numpy.reshape(
                numpy.fromstring(image.GetData(), numpy.uint8),
...
#-- wxPython Event Handlers ----------------------------------------------------
    ...
    def _on_paint(self, event):
        ...
        wx.PaintDC(control).DrawBitmap(image, (wdx - image.GetWidth()) // 2,
                                       (wdy - image.GetHeight()) // 2, True)
    ...
    def _mouse_move(self, event):
        ...
        """
        if self._x is not None:
            if ((abs(self._x - event.GetX()) + abs(self._y - event.GetY())) >=
                    3):
                self.control.ReleaseMouse()
                self._x = None
                if self._is_file:
...
class FileDropSource(wx.DropSource):
    ...
    def __init__(self, source, files):
        ...
        for file in files:
            data_object.AddFile(file)  #.Sizer.Addile( file )
