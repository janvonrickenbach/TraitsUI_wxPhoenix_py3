...
class TabularEditor(Editor):
    ...
    def _update_editor(self, object, name, old_value, new_value):
        ...
        super(TabularEditor, self)._update_editor(object, name, old_value,
                                                  new_value)
            ...
            if selected is None:
                ...
                try:
                    self.control.SetItemState(
                        self.value.index(selected), wx.LIST_STATE_SELECTED,
    ...
    def _end_label_edit(self, event):
        ...
        """
        self.adapter.set_text(self.object, self.name,
                              event.GetIndex(),
    ...
    def wx_dropped_on(self, x, y, data, drag_result):
        ...
        # end of the list:
        if ((row == -1) and ((flags & wx.LIST_HITTEST_NOWHERE) != 0) and
    ...
    def _get_column(self, x, translate=False):
        ...
        if x >= 0:
            ...
            for i in range(control.GetColumnCount()):
                ...
                if x < 0:
                    if translate:
                        return self.adapter.get_column(self.object, self.name,
                                                       i)
