...
from traits.api import (Any, Bool, Color, Either, Enum, Event, Float, Font,
                        HasPrivateTraits, HasTraits, Instance, Int, Interface,
                        List, Property, Str, cached_property, on_trait_change,
                        provides)
        ...
        for i, adapter in enumerate(self.adapters):
            ...
            if item is not None and hasattr(item_class, '__mro__'):
                for klass in item_class.__mro__:
                    handler = (self._get_handler_for(
                        ...
                        prefix) or self._get_handler_for(
            ...
            if handler is None:
                handler = (self._get_handler_for('%s_%s' % (
                    column_id, trait_name), prefix) or
