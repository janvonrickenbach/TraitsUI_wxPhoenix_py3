...
class AffectsAverageColumn(ObjectColumn):
    ...
    menu = Menu(
        Action(
            name='Add', action='column.add(object)'),
        Action(
            name='Sub', action='column.sub(object)'))
...
player_editor = TableEditor(
    ...
    columns=[
        ObjectColumn(
            name='name', editable=False, width=0.28), AffectsAverageColumn(
                name='at_bats', label='AB'), AffectsAverageColumn(
                    name='strike_outs', label='SO'), AffectsAverageColumn(
                        name='singles', label='S'), AffectsAverageColumn(
                            name='doubles', label='D'), AffectsAverageColumn(
                                name='triples', label='T'),
        AffectsAverageColumn(
            name='home_runs', label='HR'), AffectsAverageColumn(
                name='walks', label='W'), ObjectColumn(
                    name='average',
                    label='Ave',
                    editable=False,
                    width=0.09,
...
class Team(HasStrictTraits):
    ...
    traits_view = View(
        Item(
            'players', show_label=False, editor=player_editor),
...
def random_player(name):
    ...
    p = Player(
        ...
        walks=randint(0, 50))
    return p.trait_set(at_bats=p.strike_outs + p.singles + p.doubles +
                       p.triples + p.home_runs + randint(100, 200))


# Create the demo:
demo = view = Team(players=[
    random_player(name)
    for name in [
