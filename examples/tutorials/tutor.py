...
class ASection(HasPrivateTraits):
    ...
    def _load_dirs(self):
        ...
        self._subsections = [
            ...
            for sf in [
                SectionFactory(
                    title=title_for(title), parent=self).set(path=dir)
...
class SectionFactory(HasPrivateTraits):
    ...
    def _path_changed(self, path):
        ...
        if len(self.descriptions) > 0:
            ...
            self.section = Lab(title=self.title,
                               path=path,
                               toc=self.toc,
                               parent=self.parent,
                               snippets=self.snippets,
    ...
    view = View(
        VGroup(
            HGroup(
                ...
                '_',
                Item(
                    'title', springy=True, editor=TitleEditor()),
                '_',
                Item(
                    'reload', style='custom', tooltip='Reload the tutorial'),
                show_labels=False),
            '_',
            HSplit(
                Item(
...
if __name__ == '__main__':
    ...
    tutor = Tutor(home=os.path.dirname(sys.argv[0])).set(path=path)
